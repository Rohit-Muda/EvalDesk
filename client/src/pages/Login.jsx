      ...
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-sm border border-slate-200 p-8">
        ...
        {error && (
          <p className="text-amber-600 text-sm text-center mb-4 bg-amber-50 rounded-lg p-3">
            {error === 'domain_not_allowed' ? 'Login restricted to configured domains.' : 'Authentication failed.'}
          </p>
        )}
        <a href={googleUrl} className="w-full flex items-center justify-center gap-2 rounded-xl bg-slate-800 text-white py-3 px-4 font-medium hover:bg-slate-700 transition">
          Sign in with Google
        </a>
      </div>
    </div>
